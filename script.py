    ...
    def process_pdfs(self, pdf_files: List[str], query: str, history: List[List[str]] = []) -> List[List[str]]:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ...
            ("human", "Given this history: {history} and this context: {context}, answer this question below: {query}. \
                ...
                If you don't know the answer based on history and context provided, ONLY say - I'm sorry, but Blaq will like us to only chat about your documents. \
...
def main():
    ...
    if pdf_files:
        ...
        while True:
            ...
            if query.lower() == 'q':
                break
            history = chatbot.process_pdfs(pdf_files, query, history)
            for q, a in history:
                print(f"Q: {q}")
                print(f"A: {a}")
